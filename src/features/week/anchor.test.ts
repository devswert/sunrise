...
describe("anclaTrasCambioDeDia", () => {
  it("sigue al día nuevo cuando cruza a la semana siguiente", () => {
    ...
    // pasada para siempre.
    const nueva = anclaTrasCambioDeDia(SEMANA, "2026-08-16", "2026-08-17");
    expect(nueva).not.toBeNull();
    expect(toISODate(nueva!)).toBe("2026-08-17");
  });
  ...
  it("no mueve nada si el día nuevo sigue en la semana visible", () => {
    ...
    expect(anclaTrasCambioDeDia(SEMANA, "2026-08-14", "2026-08-16")).toBeNull();
  });
  ...
  it("no mueve nada si el usuario navegó a otra semana", () => {
    ...
    expect(anclaTrasCambioDeDia(otraSemana, "2026-08-16", "2026-08-17")).toBeNull();
  });

  it("cruza varios días de golpe (una suspensión larga)", () => {
    const nueva = anclaTrasCambioDeDia(SEMANA, "2026-08-12", "2026-08-25");
    expect(toISODate(nueva!)).toBe("2026-08-25");
  });
});
