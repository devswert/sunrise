 ...
 */
import type { Category, RollupCelda } from "./types";
...
function etiquetaDe(id: number | null, cats: Map<number, Category>): Omit<Segmento, "seconds"> {
  ...
  if (!c) return { key: `cat-${id}`, nombre: "Sin nombre", color: "var(--faint)" };
  return { key: `cat-${id}`, nombre: c.name, color: `var(--${c.color})` };
}

/**
 * Suma celdas en segmentos, de mayor a menor.
 *
 * `porContexto` agrupa por la categoría raíz (`parentId ?? id`), que es como se
...
export function agrupar(
  ...
  for (const celda of celdas) {
    const base = etiquetaDe(porContexto ? celda.contextId : celda.categoryId, cats);
